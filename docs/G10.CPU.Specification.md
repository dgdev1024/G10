...
Registers `D0`, `W0` and `L0` are collectively designated as the "accumulator"
registers. However, the G10 CPU allows any of the `D*`, `W*` and `L*` registers
to act as accumulator registers for use by arithmetic, bitwise and logical
instructions, as well as certain CPU control instructions.
...
| `0x06X0 DAA LX`           | 2         | 2         | `?-0?-`           | Decimal-adjusts register `LX` for BCD (Binary-Coded Decimal) representation.                      |
...
- `DA` is an alias for all `DAA` instructions.
...
- `JMP`, `JPB`, `CALL` and `RET` instructions invoked with no execution condition
    ...
    corresponding instructions with the `NN` (No Condition) execution condition.
    (e.g. `JMP IMM32` is the same as `JMP NN, IMM32`, `RET` is the same as `RET NN`).
...
on 8-bit data. These instructions support addition and subtraction, with optional 
carry/borrow handling. Increment and decrement operations are also provided.

| Opcode & Mnemonic         | Length    | Timing    | Flags             | Description                                                                                       |
|---------------------------|-----------|-----------|-------------------|---------------------------------------------------------------------------------------------------|
| `0x50X0 ADD LX, IMM8`     | 3         | 3         | `?0???`           | Adds immediate `IMM8` to register `LX`.                                                           |
| `0x51XY ADD LX, LY`       | 2         | 2         | `?0???`           | Adds register `LY` to register `LX`.                                                              |
| `0x52XY ADD LX, [DY]`     | 2         | 3         | `?0???`           | Adds value from address in register `DY` to register `LX`.                                        |
| `0x53X0 ADC LX, IMM8`     | 3         | 3         | `?0???`           | Adds immediate `IMM8` and Carry flag to register `LX`.                                            |
| `0x54XY ADC LX, LY`       | 2         | 2         | `?0???`           | Adds register `LY` and Carry flag to register `LX`.                                               |
| `0x55XY ADC LX, [DY]`     | 2         | 3         | `?0???`           | Adds value from address in register `DY` and Carry flag to register `LX`.                         |
| `0x56X0 SUB LX, IMM8`     | 3         | 3         | `?1???`           | Subtracts immediate `IMM8` from register `LX`.                                                    |
| `0x57XY SUB LX, LY`       | 2         | 2         | `?1???`           | Subtracts register `LY` from register `LX`.                                                       |
| `0x58XY SUB LX, [DY]`     | 2         | 3         | `?1???`           | Subtracts value from address in register `DY` from register `LX`.                                 |
| `0x59X0 SBC LX, IMM8`     | 3         | 3         | `?1???`           | Subtracts immediate `IMM8` and Carry flag from register `LX`.                                     |
| `0x5AXY SBC LX, LY`       | 2         | 2         | `?1???`           | Subtracts register `LY` and Carry flag from register `LX`.                                        |
| `0x5BXY SBC LX, [DY]`     | 2         | 3         | `?1???`           | Subtracts value from address in register `DY` and Carry flag from register `LX`.                  |
...
| Opcode & Mnemonic         | Length    | Timing    | Flags             | Description                                                                                       |
|---------------------------|-----------|-----------|-------------------|---------------------------------------------------------------------------------------------------|
| `0x60X0 ADD WX, IMM16`    | 4         | 5         | `?0???`           | Adds immediate `IMM16` to register `WX`.                                                          |
| `0x61XY ADD WX, WY`       | 2         | 3         | `?0???`           | Adds register `WY` to register `WX`.                                                              |
| `0x62X0 ADD DX, IMM32`    | 6         | 9         | `?0???`           | Adds immediate `IMM32` to register `DX`.                                                          |
| `0x63XY ADD DX, DY`       | 2         | 5         | `?0???`           | Adds register `DY` to register `DX`.                                                              |
| `0x64X0 SUB WX, IMM16`    | 4         | 5         | `?1???`           | Subtracts immediate `IMM16` from register `WX`.                                                   |
| `0x65XY SUB WX, WY`       | 2         | 3         | `?1???`           | Subtracts register `WY` from register `WX`.                                                       |
| `0x66X0 SUB DX, IMM32`    | 6         | 9         | `?1???`           | Subtracts immediate `IMM32` from register `DX`.                                                   |
| `0x67XY SUB DX, DY`       | 2         | 5         | `?1???`           | Subtracts register `DY` from register `DX`.                                                       |
...
logical operations on 8-bit data. These instructions support AND, OR, XOR and 
NOT operations, as well as comparison operations.

| Opcode & Mnemonic         | Length    | Timing    | Flags             | Description                                                                                       |
|---------------------------|-----------|-----------|-------------------|---------------------------------------------------------------------------------------------------|
| `0x70X0 AND LX, IMM8`     | 3         | 3         | `?0100`           | Performs bitwise AND between immediate `IMM8` and register `LX`.                                  |
| `0x71XY AND LX, LY`       | 2         | 2         | `?0100`           | Performs bitwise AND between register `LY` and register `LX`.                                     |
| `0x72XY AND LX, [DY]`     | 2         | 3         | `?0100`           | Performs bitwise AND between value from address in register `DY` and register `LX`.               |
| `0x73X0 OR LX, IMM8`      | 3         | 3         | `?0000`           | Performs bitwise OR between immediate `IMM8` and register `LX`.                                   |
| `0x74XY OR LX, LY`        | 2         | 2         | `?0000`           | Performs bitwise OR between register `LY` and register `LX`.                                      |
| `0x75XY OR LX, [DY]`      | 2         | 3         | `?0000`           | Performs bitwise OR between value from address in register `DY` and register `LX`.                |
| `0x76X0 XOR LX, IMM8`     | 3         | 3         | `?0000`           | Performs bitwise XOR between immediate `IMM8` and register `LX`.                                  |
| `0x77XY XOR LX, LY`       | 2         | 2         | `?0000`           | Performs bitwise XOR between register `LY` and register `LX`.                                     |
| `0x78XY XOR LX, [DY]`     | 2         | 3         | `?0000`           | Performs bitwise XOR between value from address in register `DY` and register `LX`.               |
...
| `0x7DX0 CMP LX, IMM8`     | 3         | 3         | `?1???`           | Compares immediate `IMM8` with register `LX`.                                                     |
| `0x7EXY CMP LX, LY`       | 2         | 2         | `?1???`           | Compares register `LY` with register `LX`.                                                        |
| `0x7FXY CMP LX, [DY]`     | 2         | 3         | `?1???`           | Compares value from address in register `DY` with register `LX`.                                  |
...
| `0x81X0 SLA HX`           | 2         | 2         | `?00?-`           | Shifts bits in register `HX` left arithmetically.                                                 |
| `0x82X0 SLA [DX]`         | 2         | 4         | `?00?-`           | Shifts bits at address in register `DX` left arithmetically.                                      |
| `0x83X0 SRA LX`           | 2         | 2         | `?00?-`           | Shifts bits in register `LX` right arithmetically.                                                |
| `0x84X0 SRA HX`           | 2         | 2         | `?00?-`           | Shifts bits in register `HX` right arithmetically.                                                |
| `0x85X0 SRA [DX]`         | 2         | 4         | `?00?-`           | Shifts bits at address in register `DX` right arithmetically.                                     |
| `0x86X0 SRL LX`           | 2         | 2         | `?00?-`           | Shifts bits in register `LX` right logically.                                                     |
| `0x87X0 SRL HX`           | 2         | 2         | `?00?-`           | Shifts bits in register `HX` right logically.                                                     |
| `0x88X0 SRL [DX]`         | 2         | 4         | `?00?-`           | Shifts bits at address in register `DX` right logically.                                          |
| `0x89X0 SWAP LX`          | 2         | 2         | `?000-`           | Swaps upper and lower nibbles in register `LX`.                                                   |
| `0x8AX0 SWAP HX`          | 2         | 2         | `?000-`           | Swaps upper and lower nibbles in register `HX`.                                                   |
| `0x8BX0 SWAP [DX]`        | 2         | 4         | `?000-`           | Swaps upper and lower nibbles at address in register `DX`.                                        |
| `0x8CX0 SWAP WX`          | 2         | 3         | `?000-`           | Swaps upper and lower bytes in register `WX`.                                                     |
| `0x8DX0 SWAP DX`          | 2         | 5         | `?000-`           | Swaps upper and lower words in register `DX`.                                                     |
...
| `0x92X0 RL HX`            | 2         | 2         | `?00?-`           | Rotates bits in register `HX` left through Carry.                                                 |
| `0x93X0 RL [DX]`          | 2         | 4         | `?00?-`           | Rotates bits at address in register `DX` left through Carry.                                      |      
| `0x9400 RLCA`             | 2         | 2         | `000?-`           | Rotates bits in accumulator `L0` left.                                                            |
| `0x95X0 RLC LX`           | 2         | 2         | `?00?-`           | Rotates bits in register `LX` left.                                                               |
| `0x96X0 RLC HX`           | 2         | 2         | `?00?-`           | Rotates bits in register `HX` left.                                                               |
| `0x97X0 RLC [DX]`         | 2         | 4         | `?00?-`           | Rotates bits at address in register `DX` left.                                                    |
| `0x9800 RRA`              | 2         | 2         | `000?-`           | Rotates bits in accumulator `L0` right through Carry.                                             |
| `0x99X0 RR LX`            | 2         | 2         | `?00?-`           | Rotates bits in register `LX` right through Carry.                                                |
| `0x9AX0 RR HX`            | 2         | 2         | `?00?-`           | Rotates bits in register `HX` right through Carry.                                                |
| `0x9BX0 RR [DX]`          | 2         | 4         | `?00?-`           | Rotates bits at address in register `DX` right through Carry.                                     |
| `0x9C00 RRCA`             | 2         | 2         | `000?-`           | Rotates bits in accumulator `L0` right.                                                           |
| `0x9DX0 RRC LX`           | 2         | 2         | `?00?-`           | Rotates bits in register `LX` right.                                                              |
| `0x9EX0 RRC HX`           | 2         | 2         | `?00?-`           | Rotates bits in register `HX` right.                                                              |
| `0x9FX0 RRC [DX]`         | 2         | 4         | `?00?-`           | Rotates bits at address in register `DX` right.                                                   |
...
| `0xA1XY BIT Y, HX`        | 2         | 2         | `?01--`           | Tests bit `Y` in register `HX`.                                                                   |
| `0xA2XY BIT Y, [DX]`      | 2         | 3         | `?01--`           | Tests bit `Y` at address in register `DX`.                                                        |
| `0xA3XY SET Y, LX`        | 2         | 2         | `-----`           | Sets bit `Y` in register `LX`.                                                                    |
| `0xA4XY SET Y, HX`        | 2         | 2         | `-----`           | Sets bit `Y` in register `HX`.                                                                    |
| `0xA5XY SET Y, [DX]`      | 2         | 4         | `-----`           | Sets bit `Y` at address in register `DX`.                                                         |
| `0xA6XY RES Y, LX`        | 2         | 2         | `-----`           | Resets (clears) bit `Y` in register `LX`.                                                         |
| `0xA7XY RES Y, HX`        | 2         | 2         | `-----`           | Resets (clears) bit `Y` in register `HX`.                                                         |
| `0xA8XY RES Y, [DX]`      | 2         | 4         | `-----`           | Resets (clears) bit `Y` at address in register `DX`.                                              |
| `0xA9XY TOG Y, LX`        | 2         | 2         | `-----`           | Toggles bit `Y` in register `LX`.                                                                 |
| `0xAAXY TOG Y, HX`        | 2         | 2         | `-----`           | Toggles bit `Y` in register `HX`.                                                                 |
| `0xABXY TOG Y, [DX]`      | 2         | 4         | `-----`           | Toggles bit `Y` at address in register `DX`.                                                      |
...
| `0xB1XY LDI HX, [DY]`     | 2         | 3         | `-----`           | Loads value from address in register `DY` into register `HX`, then increments `DY` by 1.          |
| `0xB2XY LDD LX, [DY]`     | 2         | 3         | `-----`           | Loads value from address in register `DY` into register `LX`, then decrements `DY` by 1.          |
| `0xB3XY LDD HX, [DY]`     | 2         | 3         | `-----`           | Loads value from address in register `DY` into register `HX`, then decrements `DY` by 1.          |
| `0xB4XY STI [DX], LY`     | 2         | 3         | `-----`           | Stores value from register `LY` into address in register `DX`, then increments `DX` by 1.         |
| `0xB5XY STI [DX], HY`     | 2         | 3         | `-----`           | Stores value from register `HY` into address in register `DX`, then increments `DX` by 1.         |
| `0xB6XY STD [DX], LY`     | 2         | 3         | `-----`           | Stores value from register `LY` into address in register `DX`, then decrements `DX` by 1.         |
| `0xB7XY STD [DX], HY`     | 2         | 3         | `-----`           | Stores value from register `HY` into address in register `DX`, then decrements `DX` by 1.         |
| `0xB800 ASP SIMM8`        | 3         | 4         | `00???`           | Adds signed immediate `SIMM8` to Stack Pointer (`SP`).                                            |
| `0xB9X0 ST [DX], IMM8`    | 3         | 4         | `-----`           | Stores immediate `IMM8` into the address pointed to by register `DX`.                             |
| `0xBAX0 LASP DX, SIMM8`   | 3         | 4         | `00???`           | Loads Stack Pointer (`SP`), adjusted by signed immediate `SIMM8`, into register `DX`.             |
| `0xBB00 ISP`              | 2         | 3         | `-----`           | Increments the Stack Pointer (`SP`) by 1.                                                         |
| `0xBC00 DSP`              | 2         | 3         | `-----`           | Decrements the Stack Pointer (`SP`) by 1.                                                         |
| `0xBDX0 ASR DX`           | 2         | 5         | `?0???`           | Adds the value of the Stack Pointer (`SP`) to register `DX`.                                      |
|                           |           |           |                   |                                                                                                   |
| `0xC0X0 LD HX, IMM8`      | 3         | 3         |                   | Loads 8-bit immediate `IMM8` into register `HX`.                                                  |
| `0xC1X0 LD HX, [IMM32]`   | 6         | 7         |                   | Loads 8-bit value from address `IMM32` into register `HX`.                                        |
| `0xC2XY LD HX, [DY]`      | 2         | 3         |                   | Loads 8-bit value from address in register `DY` into register `HX`.                               |
| `0xC3X0 LDQ HX, [IMM16]`  | 4         | 5         |                   | Loads 8-bit value from address `$FFFF0000 + ADDR16` into register `HX`.                           |
| `0xC4XY LDQ HX, [WY]`     | 2         | 3         |                   | Loads 8-bit value from address `$FFFF0000 + WY` into register `HX`.                               |
| `0xC5X0 LDP HX, [IMM8]`   | 3         | 4         |                   | Loads 8-bit value from address `$FFFFFF00 + ADDR8` into register `HX`.                            |
| `0xC6XY LDP HX, [LY]`     | 2         | 3         |                   | Loads 8-bit value from address `$FFFFFF00 + LY` into register `HX`.                               |
| `0xC70Y ST [IMM32], HY`   | 6         | 7         |                   | Stores 8-bit value from register `HY` into address `IMM32`.                                       |
| `0xC8XY ST [DX], HY`      | 2         | 3         |                   | Stores 8-bit value from register `HY` into address in register `DX`.                              |
| `0xC90Y STQ [IMM16], HY`  | 4         | 5         |                   | Stores 8-bit value from register `HY` into address `$FFFF0000 + ADDR16`.                          |
| `0xCAXY STQ [WX], HY`     | 2         | 3         |                   | Stores 8-bit value from register `HY` into address `$FFFF0000 + WX`.                              |
| `0xCB0Y STP [IMM8], HY`   | 3         | 4         |                   | Stores 8-bit value from register `HY` into address `$FFFFFF00 + ADDR8`.                           |
| `0xCCXY STP [LX], HY`     | 2         | 3         |                   | Stores 8-bit value from register `HY` into address `$FFFFFF00 + LX`.                              |
| `0xCDXY MV LX, HY`        | 2         | 2         |                   | Moves 8-bit value from register `HY` to register `LX`.                                            |
|                           |           |           |                   |                                                                                                   |
| `0xD0XY ADD LX, HY`       | 2         | 2         | `?0???`           | Adds register `HY` to register `LX`.                                                              |
| `0xD1XY ADC LX, HY`       | 2         | 2         | `?0???`           | Adds register `HY` and Carry flag to register `LX`.                                               |
| `0xD2XY SUB LX, HY`       | 2         | 2         | `?1???`           | Subtracts register `HY` from register `LX`.                                                       |
| `0xD3XY SBC LX, HY`       | 2         | 2         | `?1???`           | Subtracts register `HY` and Carry flag from register `LX`.                                        |
| `0xD4X0 INC HX`           | 2         | 2         | `?0?-?`           | Increments register `HX` by 1.                                                                    |
| `0xD5X0 DEC HX`           | 2         | 2         | `?1?-?`           | Decrements register `HX` by 1.                                                                    |
| `0xD6XY AND LX, HY`       | 2         | 2         | `?0100`           | Performs bitwise AND between register `HY` and register `LX`.                                     |
| `0xD7XY OR LX, HY`        | 2         | 2         | `?0000`           | Performs bitwise OR between register `HY` and register `LX`.                                      |
| `0xD8XY XOR LX, HY`       | 2         | 2         | `?0000`           | Performs bitwise XOR between register `HY` and register `LX`.                                     |
| `0xD9X0 NOT HX`           | 2         | 2         | `?0000`           | Performs bitwise NOT on register `HX`.                                                            |  
| `0xDAXY CMP LX, HY`       | 2         | 2         | `?1???`           | Compares register `HY` with register `LX`.                                                        |

##### Notes

- For `0xB800 ASP SIMM8` and `0xBAX0 LASP DX, SIMM8`:
    - `Z`: Clear
    - `N`: Clear
    - `H`: Set if there was a carry from bit 3 to bit 4; cleared otherwise.
    - `C`: Set if there was a carry from bit 7; cleared otherwise.
    - `V`: Set if signed overflow occurred; cleared otherwise.
- For `0xB9X0 ST [DX], IMM8`:
    - No flags are affected.
- For `0xBB00 ISP` and `0xBC00 DSP`:
    - No flags are affected.
- For `0xBDX0 ASR DX`:
    - `Z`: Set if the result is zero; cleared otherwise.
    - `N`: Cleared.
    ...
    - `V`: Set if signed overflow occurred; cleared otherwise.
- For all `0xD***` instructions:
    - See their respective counterparts which work on low byte register sources
      for how flags are affected.
